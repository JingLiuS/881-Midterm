...
set.seed(50)
wait <- function(w,lam){
  if (is.numeric(w) == FALSE | is.numeric(lam) == FALSE){
    #Error handling in case w and lambda aren't input as numbers (JP)
    stop("w and lambda need to be numeric values")
  }
  a = NULL
  for(i in 1:w){
...
wait.until <- function(Max,lam){
  if (is.numeric(Max) == FALSE | is.numeric(lam) == FALSE){
    #Error handling in case max or lambda are not input as numbers (JP)
    stop("Max and lambda need to be numeric values")
  }
  time = 0
  a = NULL
  while(time < Max){
...
poi.test <- function(rep, Max, lam){
  #Error handling in case rep isn't input as a number, Max and lam are handled in the other function (JP)
  a = NULL
  for(i in 1:rep){
...
wait.for <- function(k, lam){
  #Error handling in case k and lam aren't input as numbers (JP)
  if (is.numeric(k) == FALSE | is.numeric(lam) == FALSE){
    stop("k and lambda need to be numeric values")
  }
  #Error handling in case k isn't an integer (JP)
  if (k %% 1 != 0){
    stop("the number of events, k, needs to be an integer")
  }
...
gam.test <-function(rep, k, lam ){
  #Error handling in case rep isn't an integer (JP)
  if (is.numeric(rep) == FALSE || rep %% 1 != 0){
    stop("the number of repetitions (rep) needs to be an integer")
  }
